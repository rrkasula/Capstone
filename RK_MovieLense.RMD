...
Summary of  the data set displays structure of edx  with no missing values. Movies are rated between 0.5 and 5.0, and total no.of rows = 9000055 .
```{r, echo=TRUE}


# Lists movies, users and genres on edx data set
```
...
A histogram of the data set mapping ratings and counts are below. The highest rating users give to movies are, in decreasing order 4.0, 3.0, 5.0. Overall users rate movies higher in the ratings scale. Similarly, users are more likely to rate full ratings rather than half ratings on the scale.
