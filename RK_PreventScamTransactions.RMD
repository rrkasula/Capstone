---
title: "Prevent Scam Transactions"
author: "Roja Kasula"
date: " March 01, 2022"
output:
  pdf_document:
...
\newpage
# Executive Summary
Customers are not charged for products that they did not purchase, if the credit card companies are able to recognize fraudulent payment transactions.
Data set contains transactions made by credit cards.Due to imbalancing nature of the data, many observations could be predicted as False Negative, in this case legal transactions instead of fraudulent transactions.
...
This data set presents transactions that occurred in two days, where we have **492 frauds** out of **284,807 transactions**. It is highly unbalanced, the positive class (frauds) account for 0.172% of all transactions. 
Due to legal ramifications detail data is not published .
The data set contains only numerical input variables which are the result of a PCA transformation. Features V1, V2, ... V28 are the principal components obtained with PCA, the only features which have not been transformed with PCA are 'Time' and 'Amount'.
...
# Split the data set into train, test data set and cv
...
# Clone the credit card data frame
...
output <- data.frame(
  Model = "Naive Baseline - Predicts Legal", 
...
```{r, echo=FALSE, include=TRUE}
# Create a Naive Bayes Model, to improve the results in AUC and AUCPR
...
## XGBoost- eXtreme Gradien Boosting
...
feature_imp_xgb <- xgb.importance(feature_names=colnames(train$feature_names), model = xgb_model)
